...
const ZuckJS = function(timeline, options) {
  ...
  const ZuckModal = () => {
    ...
    const createStoryViewer = function(storyData, className, forcePlay) {
      ...
      storyData.timeAgo =
        storyItems && storyItems[0] ? timeAgo(get(storyItems[0], "time")) : "";
      ...
      storyViewer.querySelector(
        ".slides-pointers .wrap"
      ).innerHTML = pointerItems;
      ...
      if (className === "viewing") {
        playVideoItem(
          storyViewer,
          storyViewer.querySelectorAll(`[data-index="${currentItem}"].active`),
          false
        );
      }
